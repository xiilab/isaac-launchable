...
class CameraFollower:
    """Per-step exponential lerp toward fleet centroid.

    Quadrupeds.py and IsaacLab's standalone demos use a static camera; replay
    needs follow because the trained policy moves robots forward unboundedly.
    Lerping every sim step (instead of teleporting every N steps) avoids the
    NVST keyframe spikes that look like buffering jitter.
    """

    def __init__(self, num_robots: int, spacing: float, device: str,
                 alpha: float = 0.02):
        # Wider offset for larger swarms.
